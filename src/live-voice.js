...
export class GeminiLiveVoice{
  ...
  async start(){
    ...
    await new Promise((resolve,reject)=>{
      ...
      ws.onopen=()=>ws.send(JSON.stringify({setup:{model:`models/${LIVE_MODEL}`,generationConfig:{responseModalities:['AUDIO'],speechConfig:{voiceConfig:{prebuiltVoiceConfig:{voiceName:'Kore'}}},thinkingConfig:{thinkingLevel:'minimal'}},systemInstruction:{parts:[{text:'اسمك ناڤ. أنتِ مساعدة شخصية سعودية بصوت أنثوي شاب وواضح وطبيعي. تكلمي بلهجة سعودية خفيفة، بسرعة محادثة طبيعية، وبجمل قصيرة. لا تستخدمين فصحى ثقيلة ولا نبرة آلية. إذا قاطعك نواف، اوقفي كلامك فورًا واسمعي آخر كلامه وردي عليه. لا تعيدين السؤال، ولا تتكلمين من نفسك بدون ما تسمعينه، ولا تطولين الصمت بين الأدوار.'}]},inputAudioTranscription:{},outputAudioTranscription:{},realtimeInputConfig:{automaticActivityDetection:{disabled:false,prefixPaddingMs:120,silenceDurationMs:420}}}}));
